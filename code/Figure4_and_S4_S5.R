...
FigS4E_plot2 <- ggplot(MRL_diff, aes(x = auc_diff, y = NC_1h)) +
  stat_density_2d(geom = "path", color = "#99c3e5") +
...
FigS4E_plot3 <- ggplot(MRL_diff, aes(x = auc_diff, y = NC_2h)) +
  stat_density_2d(geom = "path", color = "#99c3e5") +
...
FigS5A_p_mRNA_2h <- ggplot(results, aes(x = mRNA_fold_change_2h, y = -log10(mRNA_p_value_2h), color = significance_2, label = ifelse(significance_2 != "Not Significant", gene_name, ""))) +
...
FigS5A_p_mRNA_12h <- ggplot(results, aes(x = mRNA_fold_change_12h, y = -log10(mRNA_p_value_12h), color = significance_12, label = ifelse(significance_12 != "Not Significant", gene_name, ""))) +
  geom_point(alpha = 0.5) +
  scale_color_manual(values = c("Increase" = "#bd0026", "Decrease" = "#253494", "Not Significant" = "grey")) +
  theme_bw() +
  theme(
...
FigS5A_p_mRNA_24h <- ggplot(results, aes(x = mRNA_fold_change_24h, y = -log10(mRNA_p_value_24h), color = significance_24, label = ifelse(significance_24 != "Not Significant", gene_name, ""))) +
  geom_point(alpha = 0.5) +
  scale_color_manual(values = c("Increase" = "#bd0026", "Decrease" = "#253494", "Not Significant" = "grey")) +
  theme_bw() +
  theme(
...
FigS5A_p_protein_2h <- ggplot(results, aes(x = protein_fold_change_2h, y = -log10(protein_p_value_2h), color = significance_2p, label = ifelse(significance_2p != "Not Significant", gene_name, ""))) +
  geom_point(alpha = 0.5) +
  scale_color_manual(values = c("Increase" = "#bd0026", "Decrease" = "#253494", "Not Significant" = "grey")) +
  theme_bw() +
  theme(
...
FigS5A_p_protein_12h <- ggplot(results, aes(x = protein_fold_change_12h, y = -log10(protein_p_value_12h), color = significance_12p, label = ifelse(significance_12p != "Not Significant", gene_name, ""))) +
  geom_point(alpha = 0.5) +
  scale_color_manual(values = c("Increase" = "#bd0026", "Decrease" = "#253494", "Not Significant" = "grey")) +
  theme_bw() +
  theme(
...
FigS5A_p_protein_24h <- ggplot(results, aes(x = protein_fold_change_24h, y = -log10(protein_p_value_24h), color = significance_24p, label = ifelse(significance_24p != "Not Significant", gene_name, ""))) +
  geom_point(alpha = 0.5) +
  scale_color_manual(values = c("Increase" = "#bd0026", "Decrease" = "#253494", "Not Significant" = "grey")) +
  theme_bw() +
  theme(
...
print(FigS5A_p_mRNA_2h)
print(FigS5A_p_mRNA_12h)
print(FigS5A_p_mRNA_24h)
print(FigS5A_p_protein_2h)
print(FigS5A_p_protein_12h)
print(FigS5A_p_protein_24h)
...
Fig4J_protein_ratio_comparison <- ggplot(plot_data_fig4j, aes(x = group, y = protein_ratio, fill = group)) +
  geom_boxplot(outlier.shape = NA) +
  coord_cartesian(ylim = c(min(plot_data_fig4j$protein_ratio, na.rm = TRUE), y_axis_max)) +
  theme_minimal() +
  labs(title = "Comparison of Protein Ratios", x = "Group", y = "Protein Ratio") +
  stat_compare_means(method = "t.test", label = "p.format", label.y = label_y_position) +
  theme(legend.position = "none") +
  scale_fill_manual(values = c("Lower" = "#1d91c0", "Upper" = "#f768a1"))

# --- Display the Plot ---
print(Fig4J_protein_ratio_comparison)
...
FigS5C_grouped_plot <- ggplot(MRL_LPS_diff_2_long, aes(x = Condition, y = MRL_diff, fill = Cluster)) +
  geom_boxplot() +
  theme_bw() +
  theme(
    axis.title = element_text(size = 14),
...
# Ensure this file path is correct for your environment.
load("/storage/cbq/cbq_backup2/disk3_old/NGS_data/20231027_integrate_data/THP1_preprocessed.RData")
...
load("/storage/disk4/cbq/NGS_data/20241030_Figures_summary/THP1_m6A_IP_seq/MRL_LPS_diff.RData")
MRL_LPS_diff_all_genes <- MRL_LPS_diff
load("/storage/disk4/cbq/NGS_data/20241030_Figures_summary/THP1_m6A_IP_seq/MRL_LPS_diff_2h_for_motif_enrichment.RData")
MRL_LPS_diff_with_protein_select <- MRL_LPS_diff
gene_id_name_pairs <- read_csv("/storage/disk4/cbq/NGS_data/20241030_Figures_summary/gene_reference/gene_id_name_pairs.csv")
...
# Create the plot
FigS4H_p1 <- ggplot(MRL_LPS_diff_m6A, aes(x = log2_fold_change_m6A, y = LPS_2h - NC_2h)) +
...
FigS4H_p3 <- ggplot(filtered_data, aes(x = group, y = log2_fold_change_m6A, fill = group)) +
...
load("/storage/disk4/cbq/NGS_data/20241030_Figures_summary/figure4/THP1_LPS_coupling_change/identified_isoform_cleaned.RData")
...
load("/storage/disk4/cbq/NGS_data/20241030_Figures_summary/figure4/THP1_LPS_coupling_change/MRL_LPS_diff_2h_for_motif_enrichment.RData")
GRCh38_gene_name_id <- read_csv("/storage/disk4/cbq/NGS_data/20241030_Figures_summary/figure2/isoform_level_RNA_features/data/GRCh38_gene_name_id.csv")
...
FigS4G_plot_motif_pattern <- ggplot(combined_results, aes(x = group, y = ratio, fill = group)) +
  geom_boxplot() +
...
combined_results <- combined_results %>%
  mutate(part = factor(part, levels = c("UTR5", "CDS", "UTR3")))
# Create subfigures for each sequence part with custom colors
FigS4G_plot_RRACH_pattern <- ggplot(combined_results, aes(x = group, y = ratio, fill = group)) +
  geom_boxplot() +
  facet_wrap(~part, scales = "free_y") +
  theme_bw() +  # Use a minimal theme
  theme(
