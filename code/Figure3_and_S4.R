...
FigS4A_plots_part1 <- plot_list

write.csv(RNA_feature_high_coupling, file = "FigS4A_high.csv", quote = F, row.names = F)
write.csv(RNA_feature_low_coupling, file = "FigS4A_low.csv", quote = F, row.names = F)
...
write.csv(cliffs_deltas_df, file = "Fig3B.csv", quote = F, row.names = F)
...
write.csv(roc_data, file = "Fig3C.csv", quote = F, row.names = F)
...
FigS4E <- ggplot(importance_df, aes(x = reorder(Feature, Importance), y = Importance)) +
...
write.csv(importance_df, file = "FigS4C.csv", quote = F, row.names = F)
...
write.csv(importance_df_xgb, file = "Fig3E.csv", quote = F, row.names = F)
...
write.csv(data_for_plot, file = "Fig3D.csv", quote = F, row.names = F)
...
write.csv(MRL_STM2457_diff, file = "Fig3F.csv", quote = F, row.names = F)
...
write.csv(data_for_plot_cds, file = "Fig3G_cds.csv", quote = F, row.names = F)
write.csv(data_for_plot_utr5, file = "Fig3G_utr5.csv", quote = F, row.names = F)
write.csv(data_for_plot_utr3, file = "Fig3G_utr3.csv", quote = F, row.names = F)
...
write.csv(MRL_YTHDF_diff, file = "Fig3H.csv", quote = F, row.names = F)
...
cor_test_FigS4F <- cor.test(MRL_YTHDF_diff$NC_2h, MRL_YTHDF_diff$YTHDF2)
FigS4F_plot2 <- ggplot(MRL_YTHDF_diff, aes(x = NC_2h, y = YTHDF2)) +
...
write.csv(MRL_YTHDF_diff, file = "FigS4F.csv", quote = F, row.names = F)
...
# Create the plot for the second model and highlight upper and lower outliers with specified colors
cor_test_FigS4G <- cor.test(MRL_YTHDF_diff$NC_2h, MRL_YTHDF_diff$YTHDF3)
FigS4G_plot3 <- ggplot(MRL_YTHDF_diff, aes(x = NC_2h, y = YTHDF3)) +
...
write.csv(MRL_YTHDF_diff, file = "FigS4G.csv", quote = F, row.names = F)
...
write.csv(data_for_plot_utr5, file = "Fig3I_5UTR.csv", quote = F, row.names = F)
write.csv(data_for_plot_utr3, file = "Fig3I_3UTR.csv", quote = F, row.names = F)
write.csv(data_for_plot_cds, file = "Fig3I_cds.csv", quote = F, row.names = F)
...
# Example usage:
renamelist_NC30min <- 'data/HEK293T_time_points/renamelist_NC30min.csv'
renamelist_NC1h <- 'data/HEK293T_time_points/renamelist_NC1h.csv'
renamelist_NC2h <- 'data/HEK293T_time_points/renamelist_NC2h_lcd.csv'

NC1h_yeast_all_counts <- "data/HEK293T_time_points/NC1h_yeast_all_counts.csv"
NC2h_yeast_all_counts <- "data/HEK293T_time_points/NC2h_yeast_all_counts.csv" # This sample use: disk1/cbq/NGS_data/20230606_2h_newdata/NC2h_lcd_*
NC30min_yeast_all_counts <- "data/HEK293T_time_points/NC30min_yeast_all_counts.csv"

source("code/functions_HEK293_new_old_isoform_diff_2024.3.7.R")

all_samples_df_30min <- process_normalize_counts(renamelist_NC30min, NC30min_yeast_all_counts, 'data/HEK293T_time_points/mapping_NC30min/cuffnorm_counts')
all_samples_df_NC1h <- process_normalize_counts(renamelist_NC1h, NC1h_yeast_all_counts, 'data/HEK293T_time_points/mapping_NC1h/cuffnorm_counts')
all_samples_df_NC2h <- process_normalize_counts(renamelist_NC2h, NC2h_yeast_all_counts, 'data/HEK293T_time_points/mapping_NC2h_lcd/cuffnorm_counts')
...
load("data/RNA_features_transcript_level.RData")
...
FigS4B <- ggplot(data = roc_data, aes(x = specificity, y = sensitivity, color = model)) +
  geom_line(size = 1) +
  geom_abline(linetype = "dashed") +
...
write.csv(roc_data, file = "FigS4B.csv", quote = F, row.names = F)
...
write.csv(combined_results, file = "FigS4E.csv", quote = F, row.names = F)
